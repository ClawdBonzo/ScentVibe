...
struct ScanningAnimationView: View {
    ...
    private let particleCount = DevicePerformance.scaledCount(20)

    var body: some View {
        ZStack {
            Color.smBackground.ignoresSafeArea()

            // Background particles — skip entirely for reduce motion / low perf
            ...
            VStack(spacing: 32) {
                ...
                ZStack {
                    ...
                    // Scanning arc — static for reduce motion
                    Circle()
                        .trim(from: 0, to: 0.3)
                        .stroke(
                    ...
                    // Inner pulse
                    Circle()
                        .fill(Color.smEmerald.opacity(reduceMotion ? 0.12 : (pulse ? 0.08 : 0.15)))
                        .frame(width: reduceMotion ? 120 : (pulse ? 130 : 110))
                    ...
                    Image(systemName: "wand.and.stars")
                        ...
                        .scaleEffect(reduceMotion ? 1.0 : (pulse ? 1.1 : 0.95))
                }
                .drawingGroup()
                ...
                HStack(spacing: 8) {
                    ForEach(0..<3, id: \.self) { i in
                        Circle()
                            ...
                            .opacity(reduceMotion ? 1.0 : (scanLine > CGFloat(i) / 3.0 ? 1.0 : 0.3))
                    }
                }
